...
async function downloadPNG() {
  ...
  try {
    ...
    // Pull the shrine background data URI out of the live .body-wrap so we can
    // repaint it as a safe <img> inside the html2canvas clone.
    window.__SHRINE_BG__ = null;
    try {
      const bw = poster.querySelector('.body-wrap');
      if (bw) {
        const bg = getComputedStyle(bw).backgroundImage || '';
        const m = bg.match(/url\(["']?(data:image\/[^"')]+)["']?\)/);
        if (m) window.__SHRINE_BG__ = m[1];
      }
    } catch (e) { /* ignore */ }
    ...
    const canvas = await html2canvas(poster, {
      ...
      // Ensure we capture everything including absolutely positioned elements
      scrollX: 0,
      scrollY: 0,
      x: 0,
      y: 0,
      // Make sure the canvas includes all overflow content
      onclone: function(clonedDoc) {
        ...
        if (clonedPoster) {
          ...
          // Make sure golden-frame is visible
          ...
          // FIX: html2canvas crashes on ANY CSS background-image it renders as a
          // pattern when it can't read a size ("createPattern ... width or height of 0").
          // Strip EVERY background-image during capture (textures + shrine photo),
          // then repaint the shrine as a real <img> layer (img elements are safe).
          const fixStyle = clonedDoc.createElement('style');
          fixStyle.textContent = `
            #poster *::before, #poster *::after { background-image:none !important; }
            #poster .poster::before { background-image:none !important; }
            #poster .body-wrap { background-image:none !important; background-color:#0d0d0d !important; }
            #poster .header::before,#poster .date-strip::before,#poster .day-band::before,#poster .hadith-strip::before{background-image:none !important;}
          `;
          clonedDoc.head.appendChild(fixStyle);

          // Repaint the shrine background as an <img> behind the body content
          const bodyWrap = clonedPoster.querySelector('.body-wrap');
          if (bodyWrap && window.__SHRINE_BG__) {
            bodyWrap.style.position = 'relative';
            const shrine = clonedDoc.createElement('img');
            shrine.src = window.__SHRINE_BG__;
            shrine.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center 25%;opacity:0.55;z-index:0;pointer-events:none;';
            bodyWrap.insertBefore(shrine, bodyWrap.firstChild);
            // lift the actual content above the shrine img
            Array.from(bodyWrap.children).forEach(ch => {
              if (ch !== shrine) { ch.style.position = ch.style.position || 'relative'; ch.style.zIndex = '1'; }
            });
          }
          
          console.log('✅ Clone prepared for capture (backgrounds neutralised, shrine repainted)');
        }
      }
    });
