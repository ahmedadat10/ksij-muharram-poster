...
async function downloadPNG() {
  ...
  // Hide interactive elements in the live view (cloned as hidden by html2canvas)
  ...
  // Build html2canvas options. `flatten = false` keeps the full design.
  // `flatten = true` is the safe fallback that strips background-images
  // (the cause of the "createPattern ... width or height of 0" crash).
  function buildOptions(flatten) {
    return {
      scale: 2,
      // null = transparent fill so the poster's own background shows through.
      // On the flattened fallback we fill with the dark theme colour instead.
      backgroundColor: flatten ? '#1a1030' : null,
      width: 1080,
      windowWidth: 1080,
      logging: false,
      useCORS: true,
      ...
      onclone: function (clonedDoc) {
        ...
        // Belt-and-braces: make sure no edit controls sneak into the image.
        clonedDoc.querySelectorAll(
          '.add-programme-btn, .add-notice-btn, .add-item-btn, .add-prayer-btn, .delete-notice, button'
        ).forEach(b => { b.style.display = 'none'; });

        const win = clonedDoc.defaultView;
        clonedPoster.querySelectorAll('*').forEach(el => {
          const cs = win.getComputedStyle(el);
          if (!cs.backgroundImage || cs.backgroundImage === 'none') return;
          // Flatten path: remove every background-image.
          // Design-preserving path: only remove ones on zero-size elements,
          // which are the invisible decorations that trigger the crash.
          if (flatten || el.offsetWidth === 0 || el.offsetHeight === 0) {
        ...
        console.log('✅ Clone prepared for capture (flatten=' + flatten + ')');
      }
    };
  }
  ...
  try {
    ...
    let canvas;
    try {
      // 1) First attempt: keep the full design intact.
      canvas = await html2canvas(poster, buildOptions(false));
    } catch (firstError) {
      // 2) If a broken background still crashes it, retry flattened.
      console.warn('Full-design capture failed, retrying flattened:', firstError.message);
      canvas = await html2canvas(poster, buildOptions(true));
    }
