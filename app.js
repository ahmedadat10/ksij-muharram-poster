...
async function downloadPNG() {
  ...
  // Read shrine background up front (to repaint the shrine safely as an <img>)
  let shrineUri = null;
  try {
  ...
  // html2canvas 1.4.1 crashes ("createPattern ... width or height of 0") when it
  // renders CSS gradients/background-images. We override each gradient element with a
  // faithful solid colour using !important, injected AFTER the template's own <style>
  // so it wins the cascade. The look stays virtually identical.
  ...
  function makeOptions(includeShrine) {
    return {
      ...
      onclone: function(clonedDoc) {
        ...
        // 1) Capture the shrine jpeg uri, then remove ALL background-image declarations
        //    (the SVG textures + the shrine photo) from the template's own <style> text.
        ...
        // 2) Inject the solid-colour overrides for every gradient (wins via !important + order).
        const fix = clonedDoc.createElement('style');
        fix.textContent = GRADIENT_FIX_CSS;
        cp.appendChild(fix);

        // 3) Clear any inline gradient/image backgrounds on elements, just in case.
        ...
        // 4) Repaint the shrine as a real <img> behind the body content (img can't crash createPattern).
        ...
        if (bodyWrap) {
          ...
          if (includeShrine && src) {
            ...
            img.src = src;
            img.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center 25%;opacity:0.55;z-index:0;pointer-events:none;';
        ...
        console.log('Clone prepared (gradients flattened, includeShrine=' + includeShrine + ')');
      }
    };
  }
  ...
  try {
    ...
    let canvas;
    try {
      canvas = await html2canvas(poster, makeOptions(true));   // with shrine
    } catch (innerErr) {
      console.warn('Capture with shrine failed, retrying without shrine:', innerErr);
      canvas = await html2canvas(poster, makeOptions(false));  // fallback: solid dark body
    }
