    ...
    const canvas = await html2canvas(poster, {
      ...
      onclone: function(clonedDoc) {
        ...
        if (clonedPoster) {
          ...
          // FIX: html2canvas crashes on inline-SVG texture backgrounds that have no
          // explicit pixel size ("createPattern ... width or height of 0").
          // Neutralise ONLY the decorative SVG texture layers; keep the real shrine photo
          // (which is a JPEG data URI) and all solid colours/gradients.
          const fixStyle = clonedDoc.createElement('style');
          fixStyle.textContent = `
            #poster::before{background-image:none !important}
            #poster .header::before,
            #poster .date-strip::before,
            #poster .day-band::before,
            #poster .hadith-strip::before{background-image:none !important}
          `;
          clonedDoc.head.appendChild(fixStyle);
          
          console.log('✅ Clone prepared for capture (SVG textures neutralised)');
        }
      }
    });
